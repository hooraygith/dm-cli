...
if (!shell.test('-f', process.cwd() + "/build/webpack.config.server.js")) {
    console.log(chalk.red('找不到文件：' + process.cwd() + "/build/webpack.config.server.js"));
    process.exit(1);
}
...
config.entry.index.unshift("webpack-dev-server/client?http://" + serverConfig.host + "/", "webpack/hot/dev-server");
